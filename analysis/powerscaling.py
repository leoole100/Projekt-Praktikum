...
plt.plot(F/xscale, P_range/yscale, label=f"400-900 nm")

# plt.plot(F/xscale, model(F, *popt_tot)/yscale, "--", color="gray", 
#     label=r"$U_\text{abs}^{" + f"{popt_tot[1]:.2f}" + r"}$"
# )


plt.axvline(1.35, color="gray", label="experiment")
...
plt.ylim(1e-6, None)
plt.xlim(F.min()/xscale, F.max()/xscale)

ax2 = plt.gca().twinx()
ax2.plot(F/xscale, Tmax, "--", color="C2")
ax2.set_ylabel(r"$T_\text{max}$ (K)", color="C2")
